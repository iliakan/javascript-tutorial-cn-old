...
同一个 promise 的所有 `.then` 都会返回相同的结果 -- 这个 promise 的结果。所以上图的代码的所有 `alert` 都会返回相同的结果： `1`。它们之间没有传递结果。
...
第一个 `.then` 显示 `1` 然后在 `(*)` 处返回 `new Promise(…)` 。一秒钟之后执行成功，然后它的结果（ `resolve` 的参数，在这儿就是 `result*2`）被传递给在 `(**)` 的第二个 `.then`。它会显示 `2`，然后做了相同的事情。
...
请注意，技术上来说也可以直接在每个 promise 后面写 `.then` 而不是返回它们，就像这样：
...
第三方库可以设计它自己的 "可兼容 promise " 对象。它们可以扩展设置方法，同时可以兼容原生的 promises ，因为它们使用了 `.then`。
...
这让扩展计划任务变为可能。即使我们现在没有扩展链的计划，我们之后可能会需要它。
...
这里的 `.catch` 正常完成。所以下一个成功的处理者被执行。或者它会返回一些东西，没什么区别。
...
*!*
    throw error; // 抛出这个或者其它错误，跳到下一个 catch
*/!*
  }
...
目前来说，不管服务器是挂了还是返回任何东西，我们的代码都会尝试将响应当做 JSON 对待。运行上面的代码你就会发现问题，因为 `no-such-user.json` 根本不存在。
...
1. 我们写了一个自定义 class 来区别 HTTP 错误和其它类型的错误。另外，这个 class 的构造函数接受 `response` 对象作为参数，然后把它保存进错误。所以错误处理代码可以进入它。
...
2. 否则，如果发生了一个错误，我们会在检查它 `(2)` 。只有是 HTTP 错误并且状态是404的时候，我们会让用户再次输入。至于其它的错误 -- 我们不知道怎样处理它， 所以我们就重新抛出它。
...
大部分的 JavaScript 引擎会追踪这种情况，然后生成一个全局的错误。我们可以在 console 中打印出来。
...
在像 Node.JS 这样的非浏览器环境下会有一些其它的类似的方法来追踪没有被处理的错误。

## 总结

总而言之，`.then/catch(handler)` 会根据错误处理者的不同返回不同的新 promise：
...
我们应该用 `unhandledrejection` 事件去处理那些在我们处理范围之外的错误（就浏览器环境而言，其它环境有类似的方法）。这样的错误通常是无法回收的，所以我们最好的处理方式是，将错误告诉给用户，同时也尽可能报告给服务器。
